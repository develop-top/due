...
func TestEncodeGetIPReq(t *testing.T) {
	buffer := protocol.EncodeBuffer(0, 0, 1, nil, protocol.EncodeGetIPReq(session.User, 3))
...
func TestEncodeGetIPRes(t *testing.T) {
	buffer := protocol.EncodeBuffer(0, 0, 1, nil, protocol.EncodeGetIPRes(codes.OK, "127.0.0.1"))
